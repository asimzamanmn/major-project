...
def update_plots():
    ...
    if samples:
        ...
        if last_data_time and (time.time() - last_data_time) > 2:
            lsl_label.setText(f"LSL Status: Stream disconnected (Last: {last_data_time})")
...
def plot_lsl_data():
    ...
    # Retry loop for stream discovery
    streams = []
    for i in range(10): # Try for 10 attempts (approx 5-10 seconds)
        streams = resolve_streams(wait_time=1.0)
        if streams:
            break
        print(f"Retry {i+1}/10: Waiting for LSL stream...")
    
    available_streams = [s.name() for s in streams]

    if not available_streams:
        print("No LSL streams found! Is the backend running and connected?")
        # Keep window open or wait? For now just return None
        input("Press Enter to exit...") # Pause so user sees message
        return None
