...
from .protocols.serial import Chords_USB    # USB protocol handler
from .protocols.wifi import Chords_WIFI     # WiFi protocol handler
from .protocols.ble import Chords_BLE       # BLE protocol handler
...
class Connection:
    ...
    def __init__(self):
        ...
        self.sample_queue = deque(maxlen=5000)     # Queue to store ALL samples for processing
    ...
    def connect_ble(self, device_address=None):
        """
        Establishes BLE connection and starts data handlers using reference event-driven model.
        """
        self.ble_connection = Chords_BLE()
        
        # Capture the original handler to chain it
        # Note: Chords_BLE.notification_handler is an instance method, 
        # but in the reference they assign a NEW function to the instance attribute.
        original_handler = self.ble_connection.notification_handler

        def notification_handler(sender, data):
            # This runs on the BLE thread
            try:
                # Initialize LSL on first packet if needed
                if not self.lsl_connection:
                     # Default to 3 channels, 500Hz as per reference
                    self.setup_lsl(num_channels=3, sampling_rate=500)
                
                # Call original (decodes data, updates samples_received)
                original_handler(sender, data)
                
                # Access the data we just decoded
                # The reference ble.py updates self.latest_sample (which I added) 
                # OR we parse it again here like the reference connection.py does.
                # Reference connection.py parses it again. Let's do that for safety.
                ...
                if len(data) == self.ble_connection.NEW_PACKET_LEN:
                    ...
                    for i in range(0, self.ble_connection.NEW_PACKET_LEN, self.ble_connection.SINGLE_SAMPLE_LEN):
                         sample_chunk = data[i:i+self.ble_connection.SINGLE_SAMPLE_LEN]
                         if len(sample_chunk) == self.ble_connection.SINGLE_SAMPLE_LEN:
                             # Extract channels (skip counter at index 0)
                             channels = [
                                int.from_bytes(sample_chunk[i:i + 2], byteorder='big', signed=True)
                                for i in range(1, len(sample_chunk), 2)
                             ]
                             
                             self.last_sample = channels
                             self.sample_queue.append(channels)
                             self.ble_samples_received += 1
                             self.samples_received += 1 # Update global counter
                ...
                elif len(data) == self.ble_connection.SINGLE_SAMPLE_LEN:
                     # Single sample
                     channels = [
                        int.from_bytes(data[i:i + 2], byteorder='big', signed=True)
                ...
                print(f"Error in processing wrapper: {e}")
        ...
        try:
            if device_address:
                ...
                # Start connection in a thread because it blocks
                self.ble_connection_thread = threading.Thread(target=self.ble_connection.connect, args=(device_address,))
                self.ble_connection_thread.daemon = True
                self.ble_connection_thread.start()
            else:
                print("Address required for automated mode")
                return False

            # Wait for connection
            print("Waiting for BLE connection establishing...")
            # Use the connection_event which matches Chords_BLE
            if self.ble_connection.connection_event.wait(timeout=15):
    ...
    def _wifi_read_loop(self):
        """Internal loop to read WiFi data in a background thread."""
        try:
            print("\nWiFi Reader Started")
            while self.running:
                ...
                if isinstance(data, (bytes, list)):
                    ...
                    for i in range(0, len(data), block_size):
                        ...
                        # Store for polling access
                        self.last_sample = channel_data
                        self.sample_queue.append(channel_data)
    ...
    def connect_wifi(self):
        ...
        Non-blocking version for API usage.
        """
        ...
        # Start the data handler thread (rate checker)
        self.running = True
        
        # Start the reader thread
        self.wifi_thread = threading.Thread(target=self._wifi_read_loop)
    ...
    def cleanup(self):
        """
        Clean up resources and connections in a safe and orderly manner.
