...
class Chords_BLE:
    ...
    def __init__(self):
        ...
        self.DATA_TIMEOUT = 5.0                    # Timeout period for data reception (seconds) - Increased for stability
    ...
    found_devices_cache = {}  # Class-level cache to store found devices
    ...
    async def scan_devices(cls):
        ...
        devices = await BleakScanner.discover(timeout=2.5) # Optimized timeout
        
        # Debug: Print all found devices
        print(f"DEBUG: Found {len(devices)} total devices.")
        
        # Don't strictly clear cache, maybe update it? 
        # Actually clearing is safer to avoid stale objects, but let's keep it fresh.
        cls.found_devices_cache = {} 
        filtered = []
        
        for d in devices:
            if d.name and d.name.startswith(cls.DEVICE_NAME_PREFIX):
                print(f" - Found NPG: '{d.name}' ({d.address})")
                filtered.append(d)
                # Store with UPPERCASE address for consistent lookup
                cls.found_devices_cache[d.address.upper()] = d 
    ...
    async def async_connect(self, device_address):
        ...
        try:
            ...
            # Use cached device object if available (much faster!)
            # Normalize address to upper case for lookup
            addr_key = device_address.upper()
            device_to_connect = self.found_devices_cache.get(addr_key, None)
            
            if not device_to_connect:
                # If not in cache (e.g. manual connect without scan), use address string
                print(f"Device not found in cache. Using address string: {device_address}")
                device_to_connect = device_address

            # Attempt Connection
            try:
                if isinstance(device_to_connect, str):
                     print(f"Connecting via Address String (Slower scan may occur)...")
                     self.client = BleakClient(device_to_connect, timeout=12.0)
                else:
                     print(f"Connecting via Cached Device Object (Fast)...")
                     self.client = BleakClient(device_to_connect, timeout=6.0) 

                await self.client.connect()
            except Exception as connect_err:
                 print(f"Initial connection failed: {connect_err}")
                 if not isinstance(device_to_connect, str):
                     print("Retrying with raw address string (Fresh Scan)...")
                     # Fallback: Ignore cache, force fresh scan by address
                     self.client = BleakClient(device_address, timeout=15.0)
                     await self.client.connect()
                 else:
                     raise connect_err # Re-raise if we already tried address string
